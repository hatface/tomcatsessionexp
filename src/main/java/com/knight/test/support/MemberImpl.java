...
public class MemberImpl implements Member, java.io.Externalizable {
     ...
     * default is false
     */
    public static final boolean DO_DNS_LOOKUPS = Boolean
        .parseBoolean(System.getProperty("org.apache.catalina.tribes.dns_lookups","false"));
    ...
    public synchronized byte[] getData(boolean getalive, boolean reset)  {
        ...
        if (dataPkg != null) {
            if (getalive) {
                ...
                XByteBuffer
                    .toBytes(alive, result, TRIBES_MBR_BEGIN.length + 4);
                dataPkg = result;
            }
            return dataPkg;
        }
